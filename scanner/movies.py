...
def _directory_source_files(
    ...
    if isinstance(configured_extensions, list):
        extensions = {
            (str(value).strip().casefold() if str(value).strip().startswith(".") else f".{str(value).strip().casefold()}")
    ...
    if isinstance(configured_ignored, list):
        ignored_names.update(str(value).strip().casefold() for value in configured_ignored)
    ...
    for source in discovered_sources:
        ...
        try:
            ...
            if parsed_items:
                ...
                message = "source_empty_or_unparseable"
        except (
            urllib.error.HTTPError,
            urllib.error.URLError,
            TimeoutError,
