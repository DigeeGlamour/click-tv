"""
Movie VOD Pipeline & Pagination Processor
...
index/page payloads for 100-item movie pagination.

The actual atomic filesystem write is intentionally handled later by scanner/output.py.
"""
...
_CATEGORY_PRIORITY = {
    category: index for index, category in enumerate(VALID_MOVIE_CATEGORIES)
}
...
def _load_required_results(file_path: str | Path) -> List[Dict[str, Any]]:
    """Load the required BD verification results without silently hiding errors."""
    path = Path(file_path)
    if not path.exists():
...
def _movie_identity(item: Dict[str, Any]) -> str:
    """
    Return a stable movie identity for category conflict resolution.
    Normalized IDs from normalizer.py are preferred.
    """
...
def _resolve_category_precedence(
    ...
    """
    Resolve conflicting source categories with this exact priority:
    Dubbed > Bangla > Hindi > South Indian > English > Mix
    """
    ...
    for item in candidates:
        ...
        if _CATEGORY_PRIORITY[category] < _CATEGORY_PRIORITY[current]:
            chosen_category[identity] = category
...
def _movie_sort_key(movie: Dict[str, Any]) -> Tuple[str, str, str]:
    """Keep pagination deterministic between scanner runs."""
...
def paginate_movie_list(
    ...
    """
    Build one index.json payload and page-XXX.json payloads for a category.
    """
...
def process_movies(
    ...
    """
    Process all movie candidates and return index/page payloads for the six
    ...
    for item in candidates:
        ...
        if pipeline == "movies":
            movie_candidates.append(dict(item))
