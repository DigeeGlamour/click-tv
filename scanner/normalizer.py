...
class Normalizer:
    ...
    def resolve_pipeline(self, candidate: Dict[str, Any]) -> str:
        ...
        if pipeline == "manual":
            if group_title.startswith("tv:"):
                return "tv"
            if group_title.startswith("movie:") or group_title == "movie" or group_title == "movies":
                return "movies"
    ...
    def detect_movie_category(self, name: str, group_title: str = "", url: str = "", default_cat: str = "Mix") -> str:
        ...
        # 1. Dubbed
        if any(t in tokens for t in ["dubbed", "dub", "dualaudio", "multiaudio"]) or "dual audio" in combined_lower or "multi audio" in combined_lower or "hindi dubbed" in combined_lower:
            return "Dubbed"

        # 2. Bangla
        if any(t in tokens for t in ["bangla", "bengali", "natok", "telefilm", "chorki", "hoichoi"]):
            return "Bangla"

        # 3. Hindi
        if any(t in tokens for t in ["hindi", "bollywood"]):
            return "Hindi"

        # 4. South Indian
        if any(t in tokens for t in ["south", "tamil", "telugu", "malayalam", "kannada"]):
            return "South Indian"

        # 5. English
        if any(t in tokens for t in ["english", "hollywood"]):
            return "English"
    ...
    def normalize_candidate(self, candidate: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        pipeline = self.resolve_pipeline(candidate)
        ...
        if pipeline == "movies":
            category = self.detect_movie_category(
                ...
                group_title,
                url,
                default_cat=candidate.get("default_category", "Mix"),
            )
        elif pipeline in ("today_match", "upcoming"):
            category = candidate.get("force_output") or pipeline
        else:
            category = self.detect_tv_category(
                clean_name,
                group_title,
                url,
                mode=candidate.get("category_mode", "detect"),
                force_cat=candidate.get("force_category", ""),
            )

        headers = self.resolve_headers(url, candidate.get("headers", {}))
        logo = self.normalize_logo(candidate.get("logo", ""))

        normalized_item = dict(candidate)
