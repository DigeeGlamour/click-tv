...
def _publish_channel_category(
    ...
    should_preserve = (
        previous_count >= minimum_baseline_count
...
def publish_scan_outputs(
    ...
    if telegram_enabled:
        ...
        message = (
            ...
            f"<b>Upcoming Matches:</b> "
            f"{scan_summary['totals']['upcoming']}\n"
            f"<b>Quarantined Channels:</b> "
            f"{len(quarantine_items)}\n"
            f"<b>Rejected Low Quality:</b> "
            f"{len(rejected_items)}\n"
            f"<b>Source Errors:</b> {len(source_errors)}"
        )
