...
const LIVE_CHANNEL_STALL_FAILOVER_MS = 28000;
const LIVE_EVENT_STALL_FAILOVER_MS = 32000;
const MOVIE_STALL_FAILOVER_MS = 12000;
...
const movieAudioCompanion = $('movieAudioCompanion');
...
function networkProfile(mode, isMovie, _fastStart = false) {
  ...
  const isEvent = !isMovie && (state.currentItem?._sourceKind === VIEW.EVENT || state.view === VIEW.EVENT);
  ...
  // LIVE Auto is the exact old Low Delay profile from index(17).html.
  if (mode === NETWORK_MODE.AUTO) {
    return {
      label: 'Auto · Lowest Buffer', lowLatencyMode: true,
      maxBufferLength: isEvent ? 6 : 8,
      maxMaxBufferLength: isEvent ? 14 : 18,
  ...
  if (mode === NETWORK_MODE.BALANCED) {
    ...
    return {
      label: 'Fast Start', lowLatencyMode: isEvent,
      maxBufferLength: isEvent ? 7 : 10,
      maxMaxBufferLength: isEvent ? 16 : 24,
...
function hlsConfigFor(mode, isMovie, _fastStart = false) {
  ...
  return {
    ...
    maxLoadingDelay: 3.5,
    testBandwidth: true,
    abrEwmaDefaultEstimate: bandwidthEstimate > 0 ? bandwidthEstimate : (effectivePerformanceClass() === 'normal' ? 2500000 : 1200000),
    abrBandwidthFactor: 0.86,
...
function shakaConfigFor(mode, isMovie, _fastStart = false) {
  const profile = networkProfile(mode, isMovie);
  const isEvent = state.currentItem?._sourceKind === VIEW.EVENT || state.view === VIEW.EVENT;
  ...
  const rebufferingGoal = isMovie
    ...
    : isEvent
      ? (mode === NETWORK_MODE.STABLE ? 1.8 : mode === NETWORK_MODE.AUTO ? 0.9 : 0.9)
      : (mode === NETWORK_MODE.STABLE ? 2 : mode === NETWORK_MODE.AUTO ? 0.9 : 1);
...
function sourceAttemptMessage() {
  ...
  if (isMatch) return retrying ? 'লাইভ ম্যাচটি চালুর আরেকটি উপায় চেষ্টা করা হচ্ছে…' : 'লাইভ ম্যাচটি চালু হচ্ছে…';
  return retrying ? 'চ্যানেলটি চালুর আরেকটি উপায় চেষ্টা করা হচ্ছে…' : 'চ্যানেলটি চালু হচ্ছে…';
}
...
async function runCurrentAttempt() {
  ...
  showPlayerMessage(sourceAttemptMessage());
...
function markAttemptProgress(reason = '', attemptToken = state.playbackSession?.attemptToken) {
  ...
  const isEvent = session.item?._sourceKind === VIEW.EVENT || state.view === VIEW.EVENT;
...
function startLiveAdaptiveQualityRamp(
  ...
  state.liveAdaptiveQualityTimer = setInterval(() => {
    ...
    const requiredBuffer = Math.max(
      2.5,
      Math.min(6, Number(profile.maxBufferLength || 0) * 0.75)
    );
    const enoughBuffer = bufferedAheadSeconds() >= requiredBuffer;
    const maximumWaitReached = totalElapsed >= 28000;

    if (!enoughBuffer && !maximumWaitReached) return;
...
function handlePlaybackSuccess() {
  const session = state.playbackSession;
  if (!session || session.id !== state.activeLoadId || session.success || String(session.attemptToken) !== video.dataset.attemptToken) return;
...
function tryLiveNetworkRecovery() {
  try {
    if (state.hls && Number(state.playbackSession?.stallStep || 0) >= 2) {
      state.hls.startLoad(-1);
    }
  } catch (_) {}
  try {
...
function startStallDetector() {
  ...
  state.stallInterval = setInterval(() => {
    ...
    if (isQualityLocked() || Date.now() < state.recoveryLockUntil) return;
    ...
    const finalRecoveryAt = kind === VIEW.EVENT ? 16000 : kind === VIEW.MOVIE ? 10000 : 9000;
    ...
    if (elapsed >= finalRecoveryAt && active.stallStep === 2) {
      active.stallStep = 3;
...
function syncMovieAudioCompanion(force = false) {
  ...
  const syncTolerance = preparedOnly ? 0.28 : 0.12;
  if (force || (Number.isFinite(target) && Math.abs(target - current) > syncTolerance)) {
    try { movieAudioCompanion.currentTime = Math.max(0, target); } catch (_) {}
  }
...
function loadMovieAudioCompanionAttempt(attempt, source, position, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    movieAudioCompanion.onloadedmetadata = () => {
      try { movieAudioCompanion.currentTime = Math.max(0, Number(position || 0)); } catch (_) {}
    };
...
async function prepareMovieAudioCompanion(item, candidate) {
  ...
  for (const attempt of attempts) {
    try {
      ...
      state.movieAudioCompanionSyncTimer = setInterval(() => syncMovieAudioCompanion(false), 250);
      return true;
    } catch (_) {
      try {
        movieAudioCompanion.removeAttribute('src');
...
function setupPlayerUi(item) {
  const isMovie = item._sourceKind === VIEW.MOVIE || state.view === VIEW.MOVIE;
  renderNetworkMenu(isMovie);
  updateNetworkMenuState(readNetworkMode(item));
  ...
  $('networkBtn').style.display = 'inline-flex';
...
video.addEventListener('seeking', () => syncMovieAudioCompanion(true));
video.addEventListener('seeked', () => syncMovieAudioCompanion(true));
...
$('progressWrapper').addEventListener('click', (event) => {
  ...
  if (!isMovie || !Number.isFinite(video.duration) || video.duration <= 0) return;
  const rect = event.currentTarget.getBoundingClientRect();
  video.currentTime = Math.max(0, Math.min(video.duration, ((event.clientX - rect.left) / rect.width) * video.duration));
});
...
function populateFullscreenDrawer(query = '') {
  ...
  const items = state.filteredItems.filter((item) => item.name.toLowerCase().includes(normalized));
  const fragment = document.createDocumentFragment();
  items.forEach((item, index) => {
    ...
    row.innerHTML = `
      <div class="fs-drawer-logo-wrap">${createImageHtml(item, '')}</div>
      <span style="font-size:11px;font-weight:700;color:#fff">#${index + 1} ${escapeHtml(item.name)}</span>`;
...
function showControlsTemporarily() {
  ...
  if ((document.fullscreenElement || innerWidth <= 1000) && !video.paused) {
...
function restoreCustomControlsAfterFullscreen() {
  const isFullscreen = document.fullscreenElement || document.webkitFullscreenElement;
  if (!isFullscreen && state.mobileNativeFullscreen) {
...
$('progressWrapper').addEventListener('touchstart', (event) => {
  seekFromPointer(event.touches[0].clientX);
}, { passive: true });
