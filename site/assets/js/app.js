...
function networkProfile(mode, isMovie, _fastStart = false) {
  ...
  if (mode === NETWORK_MODE.AUTO) {
    return {
      ...
      maxBufferLength: isEvent ? 7 : 9,
      maxMaxBufferLength: isEvent ? 16 : 20,
...
function liveStartupBufferTargetSeconds(item = state.currentItem) {
  ...
  const isEvent = item?._sourceKind === VIEW.EVENT || state.view === VIEW.EVENT;
  const proportional = Number(profile.maxBufferLength || 0) * 0.5;
  return Math.max(2.5, Math.min(isEvent ? 5 : 6, proportional));
}

function liveStartupBufferMaximumWaitMs() {
  const mode = currentNetworkMode();
  if (mode === NETWORK_MODE.BALANCED) return 2800;
  if (mode === NETWORK_MODE.STABLE) return 4500;
  return 3600;
}
...
function startLiveStartupBufferGate(session, attemptToken) {
  ...
  showPlayerMessage('Smooth playback-এর জন্য অল্প buffer প্রস্তুত হচ্ছে…', true);

  try { video.pause(); } catch (_) {}

  const targetSeconds = liveStartupBufferTargetSeconds(session.item);
  const maximumWaitMs = liveStartupBufferMaximumWaitMs();
  const check = () => {
    if (!isActiveAttempt(session, attemptToken)) {
      if (session.startupBufferGateTimer) clearInterval(session.startupBufferGateTimer);
      session.startupBufferGateTimer = null;
    ...
    if (buffered >= targetSeconds || elapsed >= maximumWaitMs) {
      releaseLiveStartupBufferGate(session, attemptToken);
    }
  };

  session.startupBufferGateTimer = setInterval(check, 160);
  check();
  return true;
}
...
function movieAudioCompanionSyncIntervalMs() {
  return effectivePerformanceClass() === 'normal' ? 500 : 750;
}
...
function syncMovieAudioCompanion(force = false) {
  ...
  const hardSyncThreshold = preparedOnly ? 0.6 : 0.45;
  ...
  } else if (!preparedOnly && Math.abs(drift) > 0.08) {
    const correction = Math.max(-0.03, Math.min(0.03, drift * 0.06));
