...
function liveStartupBufferTargetSeconds(item = state.currentItem) {
  ...
  const isEvent = item?._sourceKind === VIEW.EVENT || state.view === VIEW.EVENT;
  if (isEvent) {
    const eventProportional = Number(profile.maxBufferLength || 0) * 0.36;
    return Math.max(2.0, Math.min(2.6, eventProportional));
  }
  const proportional = Number(profile.maxBufferLength || 0) * 0.4;
  return Math.max(2.2, Math.min(3.6, proportional));
}
...
function liveStartupBufferMaximumWaitMs(item = state.currentItem) {
  ...
  const isEvent = item?._sourceKind === VIEW.EVENT || state.view === VIEW.EVENT;
  if (isEvent) {
    if (mode === NETWORK_MODE.BALANCED) return 1800;
    if (mode === NETWORK_MODE.STABLE) return 3200;
    return 2300;
  }
