...
const LIVE_CHANNEL_STALL_FAILOVER_MS = 10000;
const LIVE_EVENT_STALL_FAILOVER_MS = 16000;
...
function liveStartupBufferTargetSeconds(item = state.currentItem) {
  ...
  const isEvent = item?._sourceKind === VIEW.EVENT || state.view === VIEW.EVENT;
  const proportional = Number(profile.maxBufferLength || 0) * 0.4;
  return Math.max(2.2, Math.min(isEvent ? 3.0 : 3.6, proportional));
}

function liveStartupBufferMaximumWaitMs() {
  const mode = currentNetworkMode();
  if (mode === NETWORK_MODE.BALANCED) return 2200;
  if (mode === NETWORK_MODE.STABLE) return 3800;
  return 2800;
}
...
function startStallDetector() {
  ...
  state.stallInterval = setInterval(() => {
    ...
    const finalRecoveryAt = kind === VIEW.EVENT
