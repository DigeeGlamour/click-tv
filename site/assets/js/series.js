...
(() => {
      ...
      activeEpisodes.forEach((episode) => {
        ...
        row.innerHTML = `
          ...
          <span class="series-episode-copy">
            <strong>${escapeHtml(episodeDisplayLabel(episode))} · ${escapeHtml(episode.episode_title)}</strong>
  ...
  function decorateMetadata(item) {
    ...
    if (watching) {
      ...
      watching.textContent = `S${twoDigits(item.season_number)} · ${episodeDisplayLabel(item)} · ${item.episode_title}`;
    }
  ...
  function handleEnded() {
    ...
    prompt.innerHTML = `
      <div><small>Next Episode</small><strong>${escapeHtml(episodeDisplayLabel(next))} · ${escapeHtml(next.episode_title)}</strong></div>
